...
export function randomInt(min: number, max?: number): number {
	...
	// Lazy-seeded xorshift32 PRNG to avoid calling crypto.getRandomValues on every invocation.
	// Seeded once from crypto.getRandomValues to preserve entropy characteristics.
	// Subsequent calls are extremely fast and allocation-free.
	if (!seeded) {
